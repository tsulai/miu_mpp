...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		ACarrier fedEx;
		ACarrier usMail;		
		String desc = ""; Double weight = null; Zone zone = null;
		double minCost = 0.00;
		ACarrier lowestCarr; 
		while(true) {
			...
			while(pkCount != noOfPk) {
				...
				zone = getZone();					
				//for 1 package
				ups = new UPS(desc, weight);
				fedEx = new FedEx(desc, weight, zone);
				usMail = new USMail(desc, weight);
			...
			// lambda & stream
			//----------------	
			listLowestCarrier.stream().forEach(c-> printOutput(c));
